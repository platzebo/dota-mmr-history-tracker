...
<section class="two grid"><div class="panel"><h2>Recent matches</h2><div class="tableWrap"><table><thead><tr><th>Date</th><th>Match</th><th>Hero</th><th>Before</th><th>Δ</th><th>After</th><th>Solo</th></tr></thead><tbody id="rows"></tbody></table></div></div><div class="panel"><h2>Sync tips</h2><p class="muted"><strong>Auto sync:</strong> run <code>sync --auto --qr</code>. It imports newest matches until it reaches known data, or resumes older history from the saved cursor when a previous run stopped before the end.</p><p class="muted"><strong>Resume:</strong> each successful page saves <code>next_start_at_match_id</code>. If a run stops at the match cap, timeout, or GC error, run the same auto command later and it continues older pages automatically.</p><p class="muted">Keep Dota 2 closed while syncing and leave the default <code>--page-delay 1s</code> enabled for conservative GameCoordinator pacing.</p></div></section>
</div>
<script>
